...
st.set_page_config(
    ...
    page_icon='icon-192x192.png' # <<< Relative path to your icon
)
import streamlit as st

# Custom CSS to hide Streamlit branding and add your own
st.markdown("""
<style>
    /* Hide Streamlit footer */
    .css-1d391kg, .css-1v0mbdj {display: none;}
    
    /* Hide hamburger menu */
    .css-14xtw13 {display: none;}
    
    /* Custom header with your logo and name */
    .custom-header {
...
<div class="custom-header">
    <img class="custom-logo" src="icon-192x192.png" alt="Your Logo">
    <h1 class="custom-title">LedgerFlow</h1>
    <p class="custom-subtitle">Powered by innovation</p>
</div>
...
# Your app content
st.write("## Welcome to your completely rebranded Streamlit app!")
st.write("The Streamlit logo and name have been replaced with your custom branding.")
...
def add_expense(expense_data, current_user_id):
    ...
    try:
        ...
        st.error(f"Database Error adding expense: {e}")
    except Exception as e:
        st.error(f"An unexpected error occurred adding expense: {e}")
    finally:
        if conn:
            conn.close()
...
def update_expense(expense_id, expense_data, current_user_id):
    ...
    try:
        ...
        st.error(f"Database Error updating expense: {e}")
    except Exception as e:
        st.error(f"An unexpected error occurred updating expense: {e}")
    finally:
        if conn:
            conn.close()
...
def delete_expense(expense_id, current_user_id):
    ...
    try:
        ...
        st.error(f"Database Error deleting expense: {e}")
    except Exception as e:
        st.error(f"An unexpected error occurred deleting expense: {e}")
    finally:
        if conn:
            conn.close()
...
def set_budget(amount, current_user_id):
    ...
    try:
        ...
        st.error(f"Database Error setting budget: {e}")
    except Exception as e:
        st.error(f"An unexpected error occurred setting budget: {e}")
    finally:
        if conn:
            conn.close()
        ...
        with st.form("signup_form"):
            ...
            if signup_button:
                if not signup_username.strip() or not signup_password.strip() or not signup_password_confirm.strip():
                    ...
                    # No else here, as register_user already handles its own error messages
...
def show_main_app():
    ...
    if st.session_state.get('show_set_budget_form'):
        ...
        with st.form("set_budget_form", clear_on_submit=True):
            ...
            if submitted_budget:
                if new_budget_amount is None or new_budget_amount <= 0: # Check for invalid budget amount
                    st.error("Please enter a valid positive budget amount.")
                else:
    ...
    if st.session_state.get('show_edit_expense_form') and st.session_state.get('edit_expense_id'):
        ...
        if expense_to_edit:
            with st.form(f"edit_expense_form_{expense_to_edit['id']}"):
                ...
                if submitted_edit:
                    if edit_amount is None or edit_amount <= 0: # Validate edited amount
                        st.error("Please enter a valid positive amount for the expense.")
                    elif not edit_description.strip(): # Validate edited description
